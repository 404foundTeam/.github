...
| 이름   | 직무         | 역할 |
|--------|-------------|------|
| 🙇‍♂️ 우승연 | 팀장 / 백엔드 | - 웰컴페이지, 마이페이지, 카드뉴스 API 개발, 회원가입/로그인<br> - RDS 설계 및 Redis 기반 무료 이용 횟수 관리 <br> - AWS EC2 + Nginx 배포 및 GitHub Actions CI/CD 구축 |
| 조은진 | 백엔드       | - 스마트 리포트, 맞춤형 마케팅, 지도 이용 제휴 API 개발 <br> - 매출/리뷰 분석 모듈 구현 |
| 유재원 | 백엔드       | - 사진 촬영 가이드 API 개발 <br> - 소상공인 인터뷰 및 사용자 요구사항 조사 |
| 한종민 | 프론트엔드   | - React 기반 웰컴페이지/카드뉴스제작/사진촬영가이드/지도제휴 개발 <br> - Axios 연동 및 상태 관리 <br> - 프론트엔드 아키텍처 전반 구현 |
| 신상민 | 프론트엔드   | - React 기반 마이페이지/소개페이지/스마트리포트/맞춤형마케팅 개발 <br> - 캘린더 뷰 및 API 연동 |
